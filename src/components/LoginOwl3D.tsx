...
/* ── Elephant model with interactive animations ── */
function Elephant({
  mousePos,
  state,
}: {
  ...
  useEffect(() => {
    ...
    // Try to find head/body parts by traversing
  ...
  useEffect(() => {
    if (state === "watching" || state === "idle") {
      ...
      targetRotation.current = { x: 0.3, y: 0 }; // Look down shyly
    } else if (state === "sad") {
      sadDroop.current = 1;
      hideAmount.current = 0;
      scaleTarget.current = 0.95;
      targetRotation.current = { x: 0.15, y: 0 }; // Slight droop
    }
  }, [mousePos, state]);
  ...
  useFrame((_, delta) => {
    ...
    // Head/body rotation follows cursor
    ...
    const bobAmount = sadDroop.current > 0.5
      ? Math.sin(bodyBob.current) * 0.08  // Sad shaking
      : Math.sin(bodyBob.current) * 0.04; // Normal idle bob
