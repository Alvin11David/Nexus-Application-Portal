...
import { Mail, Phone, MapPin, Instagram, Twitter, Linkedin, Youtube, ArrowUpRight } from "lucide-react";
...
const Footer = () => {
  ...
  return (
    <footer ref={footerRef} className="bg-primary text-primary-foreground">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 lg:gap-8">
          ...
          <div className="footer-col lg:col-span-1 opacity-0">
            ...
            <div className="space-y-4">
              ...
              <a href="tel:+12125551847" className="group flex items-center gap-3 font-body text-sm text-primary-foreground/70 transition-colors duration-500 hover:text-primary-foreground">
                <Phone size={16} className="text-primary-foreground/40 group-hover:text-accent transition-colors duration-500" />
                <span className="relative">
                  +1 (212) 555-1847
                  <span className="absolute -bottom-0.5 left-0 w-full h-px bg-accent scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left" />
                </span>
              ...
              {resources.map((link) => (
                <li key={link.label}>
                  ...
                  >
                    <span className="w-0 h-px bg-accent group-hover:w-4 transition-all duration-500" />
                    {link.label}
                    <ArrowUpRight size={12} className="opacity-0 group-hover:opacity-60 transition-opacity duration-500 -ml-1" />
                  </a>
                </li>
              ))}
            </ul>
            ...
            <div className="flex gap-3 mb-10">
              {socials.map((s) => {
                ...
                return (
                  ...
                  >
                    <Icon size={16} className="text-primary-foreground/60 group-hover:text-accent transition-colors duration-500" />
                  </a>
                );
              })}
            </div>
      ...
      {/* Bottom bar */}
      <div ref={bottomRef} className="border-t border-primary-foreground/10 px-8 md:px-16 py-6 flex flex-col md:flex-row items-center justify-between gap-4 opacity-0">
